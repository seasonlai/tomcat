...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel,SocketChannel> {
    ...
    public void bind() throws Exception {
        initServerSocket();//这里就开始监听端口了

        // Initialize thread count defaults for acceptor, poller
        if (acceptorThreadCount == 0) {//接受请求线程至少为1
            // FIXME: Doesn't seem to work that well with multiple accept threads
            acceptorThreadCount = 1;
        }
        if (pollerThreadCount <= 0) {//轮询线程至少为1
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            running = true;
            paused = false;
            //会对processor、event、nioChannel对象做缓存
    ...
    public static class PollerEvent implements Runnable {
        ...
        @Override
        public void run() {
            if (interestOps == OP_REGISTER) {//如果是注册
                try {
                    //拿到通道，并在通道上注册选择器，和感兴趣事件为读，同时绑定一个NioSocketWrapper对象
                ...
                try {
                    if (key == null) {//key为null，说明被取消那可以减少一个连接数
                        ...
                        if (socketWrapper != null) {
                            ...
                            socketWrapper.interestOps(ops);//设置感兴趣事件
                            key.interestOps(ops);//把当前事件（PollerEvent）的感兴趣事件设置到SelectionKey中
                        } else {
                            //没有绑定NioSocketWrapper对象，直接取消
    ...
    public class Poller implements Runnable {
         ...
         *
         * @param nioChannel    The newly created socket
         */
        public void register(final NioChannel nioChannel) {
            //标记好所属的poller
            nioChannel.setPoller(this);
            //包装进NioSocketWrapper
            NioSocketWrapper ka = new NioSocketWrapper(nioChannel, NioEndpoint.this);
            //标记好所属的NioSocketWrapper
            nioChannel.setSocketWrapper(ka);
            //对NioSocketWrapper进行一些设值
            ka.setPoller(this);
            ka.setReadTimeout(getConnectionTimeout());
            ka.setWriteTimeout(getConnectionTimeout());
            ka.setKeepAliveLeft(NioEndpoint.this.getMaxKeepAliveRequests());
            ka.setSecure(isSSLEnabled());
            //从缓存获取一个PollerEvent对象
            PollerEvent r = eventCache.pop();
            //设置读感兴趣
            ka.interestOps(SelectionKey.OP_READ);//this is what OP_REGISTER turns into.
            //感兴趣事件为注册事件
            if ( r==null) r = new PollerEvent(nioChannel,ka,OP_REGISTER);
            else r.reset(nioChannel,ka,OP_REGISTER);
            addEvent(r);
        }
        ...
        public NioSocketWrapper cancelledKey(SelectionKey key) {
            ...
            try {
                ...
                ka = (NioSocketWrapper) key.attach(null);//拿到之前的
                if (ka != null) {
                    // If attachment is non-null then there may be a current
                    // connection with an associated processor.
                    getHandler().release(ka);//实际拿到ka对应processor进行释放处理
                }
                //关闭通道
                if (key.isValid()) key.cancel();//设置为取消状态
            ...
            while (true) {
                ...
                try {
                    if (!close) {
                        hasEvents = events();
                        /**
                         * @see #addEvent(PollerEvent)
                         */
                        if (wakeupCounter.getAndSet(-1) > 0) {
                            //if we are here, means we have other stuff to do
                            //do a non blocking select
                            keyCount = selector.selectNow();
                        } else {//是-1才进入这里，会阻塞selectorTimeout时间
        ...
        protected void processKey(SelectionKey sk, NioSocketWrapper attachment) {
            try {
                if ( close ) {//关闭状态直接取消key
                    cancelledKey(sk);
                } else if ( sk.isValid() && attachment != null ) {
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {//处理文件
        ...
        public SendfileState processSendfile(SelectionKey sk, NioSocketWrapper socketWrapper,
            ...
            try {
                ...
                if (sd.fchannel == null) {
                    ...
                    sd.fchannel = fis.getChannel();//获取一个FileChannel
                }
                ...
                if (sc.getOutboundRemaining()>0) {
                    if (sc.flushOutbound()) {
                        socketWrapper.updateLastWrite();//更新上次写时间
                    }
                } else {
                    //写入通道
                    ...
                    if (written > 0) {
                        sd.pos += written;
                        sd.length -= written;
                        socketWrapper.updateLastWrite();//更新上次写入时间
                    } else {
                ...
                if (sd.length <= 0 && sc.getOutboundRemaining()<=0) {
                    ...
                    if (calledByProcessor) {
                        ...
                        reg(sk,socketWrapper,SelectionKey.OP_WRITE);//注册写事件
                    }
            ...
            try {
                for (SelectionKey key : selector.keys()) {
                    ...
                    try {
                        ...
                        if ( ka == null ) {//取消当前SelectionKey，因为没有attachment
